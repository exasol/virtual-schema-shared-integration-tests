...
public abstract class ScalarFunctionsTestBase {
    ...
    /**
     * These functions are tested separately in {@link WithNoParameters#testFunctionsWithNoParenthesis(String)} )}
     */
     ...
     */
    protected abstract Set<String> getDialectSpecificExcludes();
    ...
    private boolean isTestDisabledFor(final String function) {
        final Set<String> dialectSpecificExcludes = getDialectSpecificExcludes();
    ...
    class WithAutomaticParameterDiscovery {
        ...
        Stream<Arguments> getScalarFunctions() {
            try (final Connection exasolConnection = createExasolConnection()) {
    ...
    class WithNoParameters {
        ...
        void testFunctionsWithNoParenthesis(final String function) {
            runOnExasol(statement -> assertDoesNotThrow(
                    () -> statement.executeQuery(this.queryBuilder.buildQueryFor(function)).close()));
        }
    ...
    class WithStringParameter {
        ...
        void testPosition() throws SQLException {
            if (isTestDisabledFor("position"))
                return;
            this.testTable.initializeSingleRowWith("my long string");
            runOnExasol(statement -> {
                final String virtualSchemaQuery = this.queryBuilder
                        .buildQueryFor("POSITION('long' IN " + this.columnName + ")");
                try (final ResultSet result = statement.executeQuery(virtualSchemaQuery)) {
